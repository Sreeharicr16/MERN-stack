...
//Validate user schema using JOI
...
exports.Signup = async (req, res) => {
  try {
    const result = userSchema.validate(req.body); //validate through JOI
    ...
    if (user) {
      return res.json({
        error: true,
        message: "Email is already in use. Cannot Sign-Up",
      });
    }
    var num = await User.findOne({
    ...
    if (num) {
      return res.json({
        error: true,
        message: "Phone number is already in use. Cannot Sign-Up",
      });
    }
    ...
    result.value.phoneNumber = result.value.phoneNumber
...
exports.Login = async (req, res) => {
  try {
    ...
    // return res.send("User is now logged in successfully")
  } 
